...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didRepost:) name:MMSinaGotRepost object:nil];
...
- (void)setStatus:(Status *)s type:(AtViewType)t
{
    ...
    if (sts.retweetedStatus){
        ...
        NSString *transformStr = [HtmlString transformString:sts.retweetedStatus.text];
        ...
        input.text = [NSString stringWithFormat:@"//@%@:%@", sts.user.screenName,sts.text];
        [num setText:[[NSString alloc] initWithFormat:@"微博字数：%d",input.text.length]];
    } else {
        textBackg = [UIScrollView new];
...
- (void) Exit
{
    if (type == AtViewFromTopCoverView)
        [self dismissModalViewControllerAnimated:YES];
    else if (type == AtViewFromTopView){
...
- (void)reloadTableViewDataSource{
	...
	_reloading = YES;
    [manager repost:[NSString stringWithFormat:@"%lld",sts.statusId] content:input.text withComment:commentOrigin];
...
-(void)didRepost:(NSNotification*)sender
{
    Status *s = sender.object;
	if (s.text != nil && [s.text length] != 0) {
